<template>
  <v-container fluid class="pa-6">
    ...
    <div class="d-flex align-center mb-5">
      ...
      <div>
        <h1 class="text-h5 font-weight-bold" style="color:#1A2744;">
          {{ isEdit ? t('edit_tour') : t('new_tour') }}
        </h1>
        <p class="text-body-2 text-medium-emphasis mt-1">{{ stepTitles[currentStep - 1] }}</p>
      </div>
      <v-spacer />
      <!-- Language toggle -->
                ...
                <v-col cols="12" md="12">
                  ...
                  >
                    ...
                    <span v-if="dest.destination_id">
                      ...
                      <span v-if="allDestinations.find(d => d.id === dest.destination_id)?.airport_code">
                        ({{ allDestinations.find(d => d.id === dest.destination_id)?.airport_code }})
                      </span>
                    </span>
                  </v-alert>
                </v-col>
...
const messages = {
  ru: {
    new_tour: 'Новый тур', edit_tour: 'Редактировать тур',
    ...
    tickets: 'Билеты', visa_applicants: 'Визы', save_draft: 'Сохранить черновик',
    confirm_tour: 'Подтвердить тур', back: 'Назад', next: 'Далее', required: 'Обязательно',
  },
  en: {
    new_tour: 'New Tour', edit_tour: 'Edit Tour',
    ...
    review_submit: 'Review & Submit', tour_info: 'Tour Information',
    summary: 'Summary', destinations: 'Stops', hotels: 'Hotels',
    tickets: 'Tickets', visa_applicants: 'Visas', save_draft: 'Save Draft',
    confirm_tour: 'Confirm Tour', back: 'Back', next: 'Next', required: 'Required',
  },
}
...
function mealColor(t) { return { breakfast: 'orange', lunch: 'green', dinner: 'blue', snack: 'purple' }[t] ?? 'grey' }
function mealLabel(k) { return messages[lang.value][k] ?? k }
...
function hotelsForDest(dest) {
  ...
  return hotels.value.filter(h => h.city?.toLowerCase() === d.name?.toLowerCase() || h.city?.toLowerCase() === d.country?.toLowerCase())
}
...
async function saveTour(status) {
  ...
  try {
    ...
    ui.showSnackbar(
      status === 'confirmed' ? (lang.value === 'ru' ? 'Тур подтверждён!' : 'Tour confirmed!') : (lang.value === 'ru' ? 'Черновик сохранён' : 'Draft saved'),
      'success'
    )
    ...
    if (errors) {
      ...
      ui.showSnackbar(lang.value === 'ru' ? 'Ошибка сохранения' : 'Save failed', 'error')
    }
  } finally {
    saving.value = false
  }
...
async function loadAll() {
  try {
    const [c, d, cp, s, h, r] = await Promise.all([
      api.get('/countries'),
    ...
    countries.value = c.data.data
    allDestinations.value = d.data.data
    counterparties.value = cp.data.data || []
    staffList.value = (s.data.data || []).filter(u => u.roles?.some(r => ['manager', 'staff', 'sales'].includes(r.name ?? r)))
    hotels.value = h.data.data || []
    restaurants.value = r.data.data || []
  } catch { /* silent */ }
}
